...
export class AuthService {
  ...
  private readonly basicAuthHeader = `Basic ${btoa(
  ...
  handleAccessToken(code: string): Observable<IAccessTokenModel> {
    return this.requestToken(
      this.getTokenRequestBody(environment.grant_type.code, {
        code,
  ...
  private requestToken(body: HttpParams): Observable<IAccessTokenModel> {
    return this.http
      .post<IAccessTokenModel>(`${environment.authUrl}/oauth2/token`, body, {
        headers: {
  ...
  private storeToken(token: IAccessTokenModel) {
    localStorage.setItem(
      this.AUTH_STORAGE_KEYS.accessToken,
      token.access_token
    );
    localStorage.setItem(
      this.AUTH_STORAGE_KEYS.refreshToken,
      token.refresh_token
    );
    localStorage.setItem(
      this.AUTH_STORAGE_KEYS.expiresIn,
  ...
  getMe() {
    return this.http.get<IUserResponse>(
      `${environment.authUrl}/gamemaster/api/v1/users`
    );
  }
